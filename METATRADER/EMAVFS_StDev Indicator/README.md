...
- `EMAVFS_StDev.mq4` - Original converted MetaTrader 4 indicator source file.
- `EMAVFS_StDev_v2.mq4` - Updated version with improved MT4 color-line buffer repaint handling.
- `EMAVFS_StDev_v3.mq4` - Object-line version designed to avoid closed-bar color repaint caused by MT4 shared line-buffer endpoints.

## Recommended Version

Use `EMAVFS_StDev_v3.mq4`.

Version 3 keeps the same indicator calculation logic, but draws the colored EMAVFS line with chart objects instead of visible MT4 color-line buffers. This avoids the shared-endpoint issue where the live candle segment can visually recolor the previous closed segment.

If you want the strictest visual behavior during testing, set `DrawLiveLine` to `false`. That hides the still-forming candle line segment, so only closed-bar segments are drawn.

## What Was Converted

- MT5 indicator initialization was adapted for MT4.
- MT5 plotting functions were replaced with MT4 `SetIndex...` functions.
- MT5 `DRAW_COLOR_LINE` behavior was adapted for MT4. Version 3 draws the colored line with chart objects to avoid MT4 line-buffer endpoint repaint.
...
4. Copy `EMAVFS_StDev_v3.mq4` into the `Indicators` folder.
5. Restart MetaTrader 4, or refresh the Navigator panel.
6. Attach `EMAVFS_StDev_v3` to a chart.
...
| `DrawLiveLine` | `true` | Version 3 only. Draws the still-forming candle segment. Set to `false` to show closed segments only. |
| `MaxObjectBars` | `3000` | Version 3 only. Maximum colored line object segments to draw. `0` means all bars. |
| `InstanceId` | `A` | Version 3 only. Unique id for running multiple copies on the same chart. |

## Indicator Buffers

MT4 does not support MT5's `DRAW_COLOR_LINE` drawing mode directly.

In version 3, the visible colored EMAVFS line is drawn with chart objects to prevent live-candle endpoint repaint from changing the previous segment's visible color. The main EMAVFS value is still available through buffer `0`.

| Buffer | Description |
| ---: | --- |
| `0` | Hidden main EMAVFS value buffer, useful for `iCustom()` calls. |
| `1` | Reserved hidden compatibility buffer. |
| `2` | Reserved hidden compatibility buffer. |
| `3` | Reserved hidden compatibility buffer. |
...
For version 3, use `"EMAVFS_StDev_v3"` as the indicator name in `iCustom()`.

## Compilation

Open `EMAVFS_StDev_v3.mq4` in MetaEditor 4 and compile it. After successful compilation, MetaTrader 4 will create the corresponding `.ex4` file.
